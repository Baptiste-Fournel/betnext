...
/**
 * Source de vérité d'auth côté front. Au montage : si un token existe, on récupère l'utilisateur via
 * `GET /auth/me` (le rôle vient du serveur, jamais déduit côté client). La SÉCURITÉ est imposée par le
 * back ; ce contexte ne fait que piloter l'UX (login / rôle / logout). 401 → purge → retour login.
 */
export function AuthProvider({ children }: { children: ReactNode }): React.JSX.Element {
  ...
  const login = useCallback(async (username: string, password: string) => {
    try {
      ...
      setToken(data.token);
      setUser({ userId: data.userId, role: data.role as Role });
      setStatus('authenticated');
      return { ok: true };
    } catch {
