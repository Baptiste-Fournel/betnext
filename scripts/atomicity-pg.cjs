...
(async () => {
  ...
  await reset();
  await ds.getRepository(WalletRecord).update({ userId: 'u1' }, { balance: '5' });
  ...
  assert.strictEqual(keyRows[0].c, 0);
  await ds.getRepository(WalletRecord).update({ userId: 'u1' }, { balance: '100' });
  ...
  assert.strictEqual(await balanceOf('u1'), 120);
  assert.strictEqual((await ds.query("SELECT count(*)::int AS c FROM outbox WHERE type='BetWon'"))[0].c, 1);
  ...
  assert.strictEqual(await betStatusOf('boom1'), 'PENDING');
  assert.strictEqual(await balanceOf('u1'), 120);
  assert.strictEqual(await balanceOf('u2'), 80);
  ...
  assert.ok(both.every((r) => r.status === 'fulfilled'));
  ...
  await compliance.setDailyCap('u1', 30);
  ...
  assert.strictEqual(capRace.filter((r) => r.status === 'fulfilled').length, 1);
  assert.strictEqual(capRace.filter((r) => r.status === 'rejected').length, 1);
  assert.strictEqual((await ds.query('SELECT count(*)::int AS c FROM bets'))[0].c, 1);
  assert.strictEqual(await balanceOf('u1'), 80);
  assert.strictEqual(Number((await ds.query('SELECT staked FROM rg_daily_stakes WHERE "userId"=$1', ['u1']))[0].staked), 20);
  ...
  await idemGuarded.execute({ ...capInp, idempotencyKey: 'cap-idem', requestHash: hashOf(capInp) });
  assert.strictEqual(Number((await ds.query('SELECT staked FROM rg_daily_stakes WHERE "userId"=$1', ['u1']))[0].staked), 30);
  assert.strictEqual((await ds.query('SELECT count(*)::int AS c FROM bets'))[0].c, 1);
